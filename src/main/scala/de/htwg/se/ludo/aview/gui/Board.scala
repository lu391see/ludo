...
case class Board() extends BoxPanel(orientation = Orientation.Vertical) {
  ...
  contents += new BorderPanel {
    add(new HomeTeam("A", Color.lightGray, 56), BorderPanel.Position.West)
    add(new HomeTeam("B", Color.lightGray, 60), BorderPanel.Position.East)
  }
  contents += HorizontalFieldRow(Color.black, Color.red, 16, 26)
  contents += new BorderPanel {
    add(VerticalFieldRow(47, 55), BorderPanel.Position.East)
    add(VerticalFieldRow(27, 35), BorderPanel.Position.West)
  }
  contents += HorizontalFieldRow(Color.green, Color.yellow, 36, 46)
  contents += new BorderPanel {
    add(new HomeTeam("C", Color.lightGray, 64), BorderPanel.Position.East)
    add(new HomeTeam("D", Color.lightGray, 68), BorderPanel.Position.West)
  }
  contents += Swing.VStrut(20)
  contents += new BorderPanel {
